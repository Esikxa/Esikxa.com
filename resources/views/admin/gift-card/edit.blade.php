...
@section('content')
    ...
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Gift Cards /</span> Edit
    </h4>
    <div class="row">
        <!-- FormValidation -->
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form id="form" class="row g-3" method="POST"
                        action="{{ route('admin.gift-card.update', $model->uuid) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="col-12">
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <div class="select2-primary">
                                <select id="client_id" class="select2 form-select" name="client_id"
                                    placeholder="Select client">
                                    <option value="">Select client</option>
                                    @foreach ($clients as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('client_id',$model->client_id) == $item->id ? 'selected' : '' }}> {{ $item->title }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" placeholder="Gift Card Code" id="code"
                                    name="code" value="{{ old('code',$model->code) }}" aria-label="Gift Card Code"
                                    aria-describedby="generate-code" readonly>
                                <button class="btn btn-primary disabled" type="button" id="generate-code">Generate Gift Card
                                    Code</button>
                            </div>
                        </div>
                        <div class="col-md-6 mb-2">
                            ...
                            <input type="datetime-local" id="start_date_time" class="form-control" name="start_date_time"
                                value="{{ old('start_date_time',$model->start_date_time) }}" />
                        </div>
                        <div class="col-md-6 mb-2">
                            ...
                            <input type="datetime-local" id="end_date_time" class="form-control" name="end_date_time"
                                value="{{ old('end_date_time',$model->end_date_time) }}" />
                        </div>
                        <div class="col-md-6 mb-2">
                            ...
                            <input type="number" id="discount_value" class="form-control" min="0"
                                name="discount_value" value="{{ old('discount_value',$model->discount_value) }}" />
                        </div>
                        <div class="col-md-6 mb-2">
                            ...
                            <input type="number" id="max_amount_spend" class="form-control" min="0"
                                name="max_amount_spend" value="{{ old('max_amount_spend',$model->max_amount_spend) }}" />
                        </div>
                        <div class="col-md-6 mb-2">
                            ...
                            <input type="number" id="min_amount_spend" class="form-control" min="0"
                                name="min_amount_spend" value="{{ old('min_amount_spend',$model->min_amount_spend) }}" />
                        </div>
                        <div class="col-md-6 mb-2">
                            ...
                            <div class="select2-primary">
                                <select id="user_id" class="select2 form-select" name="user_id" placeholder="Select User">
                                    ...
                                    @foreach ($users as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('user_id', $model->users()->exists() ? $model->users->first()->id : '') == $item->id ? 'selected' : '' }}>
                                            {{ $item->full_name }}
                                        </option>
                                    @endforeach
                                </select>
