...
<script>
    $('#generate-code').click(function() {
        const couponCode = generateGiftCardcode(8);
        if (couponCode) {
            $.ajax({
                type: "GET",
...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">Gift Cards /</span> Add New
</h4>
<div class="row">
    <!-- FormValidation -->
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                <form id="form" class="row g-3" method="POST" action="{{ route('admin.gift-card.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="col-12">
                        <h6 class="fw-semibold">Gift Card</h6>
                        <hr class="mt-0" />
                    </div>
                    <div class="col-md-6 mb-2">
                        ...
                        <div class="select2-primary">
                            <select id="client_id" class="select2 form-select" name="client_id" placeholder="Select client">
                                ...
                                <option value="{{ $item->id }}" {{ old('client_id', session('client-ID')) == $item->id ? 'selected' : '' }}> {{ $item->title }}
                                </option>
                                @endforeach
                            </select>
                        </div>
        ...
        <div class="col-md-6 mb-2">
            <label class="form-label" for="code">Gift Card Code</label>
            <div class="input-group">
                <input type="text" class="form-control" placeholder="Gift Card Code" id="code" name="code" value="{{ old('code') }}" aria-label="Gift Card Code" aria-describedby="generate-code">
                <button class="btn btn-primary" type="button" id="generate-code">Generate Gift Card
                    Code</button>
            </div>
        </div>
        <div class="col-md-6 mb-2">
            <label class="form-label" for="start_date_time">Start Date Time</label>
            <input type="datetime-local" id="start_date_time" class="form-control" name="start_date_time" value="{{ old('start_date_time') }}" />
        </div>
        <div class="col-md-6 mb-2">
            <label class="form-label" for="end_date_time">End Date Time</label>
            <input type="datetime-local" id="end_date_time" class="form-control" name="end_date_time" value="{{ old('end_date_time') }}" />
        </div>
        <div class="col-md-6 mb-2">
            <label class="form-label" for="discount_value">Discount Value (in NPR)</label>
            <input type="number" id="discount_value" class="form-control" min="0" name="discount_value" value="{{ old('discount_value') }}" />
        </div>
        <div class="col-md-6 mb-2">
            <label class="form-label" for="max_amount_spend">Max. Amount Spend (in NPR)</label>
            <input type="number" id="max_amount_spend" class="form-control" min="0" name="max_amount_spend" value="{{ old('max_amount_spend') }}" />
        </div>
        <div class="col-md-6 mb-2">
            <label class="form-label" for="min_amount_spend">Min. Amount Spend (in NPR)</label>
            <input type="number" id="min_amount_spend" class="form-control" min="0" name="min_amount_spend" value="{{ old('min_amount_spend') }}" />
        </div>
        <div class="col-md-6 mb-2">
            ...
            <div class="select2-primary">
                <select id="user_id" class="select2 form-select" name="user_id" placeholder="Select User">
                    ...
                    <option value="{{ $item->id }}" {{ old('user_id') == $item->id ? 'selected' : '' }}> {{ $item->full_name }} - {{ $item?->client?->title }}
                    </option>
                    @endforeach
                </select>
            </div>
