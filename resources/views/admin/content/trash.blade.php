...
@section('content')
    ...
    <h4 class="fw-bold py-3 mb-4">
        <span class="text-muted fw-light">Content /</span> Trash
    </h4>
    ...
    <div class="card">
        <div class="card-header">
            ...
            <div class="float-end">
                <a href="{{ route('admin.content.index') }}" class="add-new btn btn-primary mb-3 mb-md-0">
                    Go Back</a>
        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                ...
                <tbody class="table-border-bottom-0">
                    @forelse ($dataProvider as $key => $item)
                        <tr>
                            <td>{{ ++$key }}</td>
                            <td><strong>{{ $item->title }}</strong></td>
                            <td><span
                                    class="badge bg-label-{{ $item->status == 1 ? 'success' : 'danger' }} me-1">{{ $item->active }}</span>
                            </td>
                            <td>

                                <a class="btn btn-icon btn-primary btn-restore"
                                    data-action="{{ route('admin.content.restore', $item->uuid) }}" href="#"
                                    title="Restore"><i class="ti ti-refresh-alert"></i></a>
                                <a class="btn btn-icon btn-danger btn-delete"
                                    data-action="{{ route('admin.content.force-delete', $item->uuid) }}" href="#"
